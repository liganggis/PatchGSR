    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration > opt.multi_view_weight_from_iter:
            ...
            if nearest_cam is not None:
                ...
                H, W = render_pkg['alpha_depth'].squeeze().shape
                ix, iy = torch.meshgrid(
                    torch.arange(W), torch.arange(H), indexing='xy')
                pixels = torch.stack([ix, iy], dim=-1).float().to(render_pkg['alpha_depth'].device)
                ...
                pts = gaussians.get_points_from_depth(viewpoint_cam, render_pkg['alpha_depth'])
                pts_in_nearest_cam = pts @ nearest_cam.world_view_transform[:3,:3] + nearest_cam.world_view_transform[3,:3]
                map_z, d_mask = gaussians.get_points_depth_in_depth_map(nearest_cam, nearest_render_pkg['alpha_depth'], pts_in_nearest_cam)
                ...
                #     depth = render_pkg['alpha_depth'].squeeze().detach().cpu().numpy()
