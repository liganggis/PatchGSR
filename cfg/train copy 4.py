...
from gaussian_renderer import render, compute_lncc
import sys, time
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # if iteration > opt.multi_view_weight_from_iter:
            ...
            if nearest_cam is not None:
                ...
                ncc, valid_indice = compute_lncc(gaussians, viewpoint_cam, nearest_cam, sample_num, pixels, render_pkg["rendered_normal"].permute(1, 2, 0), render_pkg['rendered_distance'].squeeze(), 
                                                                render_pkg['alpha_depth'], depthMaps[nearest_cam.uid][None], total_patch_size, pixel_noise_th, 
                                                    pipe, bg, kernel_size=dataset.kernel_size, return_depth=True)

                ncc_loss = ncc_weight * ncc.sum()
                loss += ncc_loss
