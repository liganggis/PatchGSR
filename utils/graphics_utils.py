...
def depths_to_points_our(view, depthmap):
    ...
    rays_d = torch.stack(
                    [(grid_x-view.Cx) / view.Fx,
                    (grid_y-view.Cy) / view.Fy,
...
def depth_to_normal_d(view, depth0, depth1):
    """
        ...
        depth: depthmap 
    """
    points0 = depths_to_points_our(view, depth0).reshape(*depth0.shape[0:], 3)
    points1 = depths_to_points_our(view, depth1).reshape(*depth1.shape[0:], 3)
...
def depth2point_cam(sampled_depth, ref_intrinsic):
    ...
    ndc_xyz = torch.stack([valid_x, valid_y, valid_z], dim=-1).view(B, N, C, H, W, 3)  # 1, 1, 1, 512, 640, 3
    cam_xyz = ndc_2_cam(ndc_xyz, ref_intrinsic, W, H) # 1, 1, 1, 512, 640, 3
